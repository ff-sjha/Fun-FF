...
import com.firstfuel.fafi.service.dto.SeasonExpenseDetails;
...
public class SeasonExpenseResource {
    ...
    public SeasonExpenseResource(SeasonExpenseService seasonExpenseService,
            SeasonExpenseQueryService seasonExpenseQueryService) {
     ...
     *
     * @param seasonExpense
     *            the seasonExpense to create
     * @return the ResponseEntity with status 201 (Created) and with body the new
     *         seasonExpense, or with status 400 (Bad Request) if the seasonExpense
     *         has already an ID
    ...
    @Timed
    public ResponseEntity<SeasonExpense> createSeasonExpense(@Valid @RequestBody SeasonExpense seasonExpense)
            throws URISyntaxException {
        log.debug("REST request to save SeasonExpense : {}", seasonExpense);
        if (seasonExpense.getId() != null) {
            throw new BadRequestAlertException("A new seasonExpense cannot already have an ID", ENTITY_NAME,
                    "idexists");
        }
     ...
     *
     * @param seasonExpense
     *            the seasonExpense to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated
     *         seasonExpense, or with status 400 (Bad Request) if the seasonExpense
     *         is not valid, or with status 500 (Internal Server Error) if the
     *         seasonExpense couldn't be updated
     * @throws URISyntaxException
     *             if the Location URI syntax is incorrect
     */
    @PutMapping("/season-expenses")
    @Timed
    public ResponseEntity<SeasonExpense> updateSeasonExpense(@Valid @RequestBody SeasonExpense seasonExpense)
            throws URISyntaxException {
     ...
     *
     * @param id
     *            the id of the seasonExpense to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the
     *         seasonExpense, or with status 404 (Not Found)
     */
     ...
     *
     * @param id
     *            the id of the seasonExpense to delete
    ...
    @GetMapping("/season-expenses/detail")
    @Timed
    public ResponseEntity<SeasonExpenseDetails> getAllSeasonExpensesDetails(SeasonExpenseCriteria criteria,
            Pageable pageable) {
        log.debug("REST request to get SeasonExpenses by criteria: {}", criteria);
        List<SeasonExpense> expenseList = seasonExpenseQueryService.findByCriteria(criteria);
        double totalExpense = expenseList.stream().mapToDouble(e -> e.getAmount()).sum();
        SeasonExpenseDetails details = new SeasonExpenseDetails(totalExpense, expenseList);
