...
import com.firstfuel.fafi.domain.News_;
...
/**
 * Service for executing complex queries for News entities in the database. The
 * main input is a {@link NewsCriteria} which get's converted to
 * {@link Specifications}, in a way that all the filters must apply. It returns
 * a {@link List} of {@link NewsDTO} or a {@link Page} of {@link NewsDTO} which
...
public class NewsQueryService extends QueryService<News> {
	...
	/**
	 * Return a {@link List} of {@link NewsDTO} which matches the criteria from the
	 * database
	 * 
	 * @param criteria
	...
	/**
	 * Return a {@link Page} of {@link NewsDTO} which matches the criteria from the
	 * database
	 * 
	 * @param criteria
	 *            The object which holds all the filters, which the entities should
