...
@Component({
    selector: 'fafi-seasons-franchise',
    templateUrl: './seasons-franchise.component.html'
})
export class SeasonsFranchiseComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/seasons-franchise'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/seasons-franchise', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    registerChangeInSeasonsFranchises() {
        this.eventSubscriber = this.eventManager.subscribe('seasonsFranchiseListModification', (response) => this.loadAll());
    }
