...
export class TournamentService {
    ...
    queryActive(req?: any): Observable<ResponseWrapper> {
        const options = createRequestOption(req);
        return this.http.get(this.resourceUrl + '?active.equals=true', options)
            .map((res: Response) => this.convertResponse(res));
    }
    ...
    private convert(tournament: Tournament): Tournament {
        ...
        copy.startDate = this.dateUtils
            .convertLocalDateToServer(tournament.startDate);
        copy.endDate = this.dateUtils
            .convertLocalDateToServer(tournament.endDate);
        return copy;
    }
}
